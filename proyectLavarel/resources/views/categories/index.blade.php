        ...
        <div>
            @foreach ($categories as $category)
                <div class="row py-1">
                    <div class="col-md-9 d-flex align-items-center">
                        <a class="d-flex align-items-center gap-2" href="{{ route('categories.show', ['category'=>$category]) }}">
                        <div style="width: 30px;height: 30px;background-color: {{$category->color}}"></div> {{ $category->name }}
                        </a>
                    </div>
            ...
            <div class="modal-dialog">
                ...
                <div class="modal-body">
                    Al eliminar la categoría <strong>{{$category->name}}</strong> se eliminarán todas las tareas asignadas a la misma.
                    ¿Está seguro de eliminar la tarea <strong>{{$category->name}}</strong>?
                </div>
